...
1. In the **Find and select** pane, ensure **Databricks Sources (1)** is selected. In the search bar, search for **`customer_transactions (2)`**, then select the asset **(3)**.

   ![Picture 1](./Media/sandbox-purview-image269.png)

1. Next, switch the collection to **Fabric Sources (1)**. Select **`dimension_customer (2)`**, then click **Add (3)**.

   ![Picture 1](./Media/sandbox-purview-image270.png)

1. Back in the **Sales Analytics** domain under **Data assets**, verify that both assets are added.
...
1. On the **Sales Analytics** domain → **Data assets** page, select each dataset one by one.

     ![Picture 1](./Media/sandbox-purview-image293.png)

1. First, select **`customer_transactions`** and review the **Overview** page:

    - Verify the **Quality score** is **High (100)**
    - Review **Active rules**, **Quality score trend**, and **Last quality scan** details

       ![Picture 1](./Media/sandbox-purview-image294.png)

1. Next, go back and select `dimension_customer`, then review the same details:
    
    - Confirm the **Quality score** is **High (100)**
    - Review the applied rule and scan results
   
        ![Picture 1](./Media/sandbox-purview-image295.png)
