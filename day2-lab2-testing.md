...
5. Click **Next** → on **Custom attributes**, skip (no changes needed) → click **Create**
6. Once created, add your lab user account as a **Domain owner** if prompted
7. Verify the `Sales Analytics` domain appears in the list

**Step 2: Create the Data Product**

7. Click **Unified Catalog** → **Catalog management** → **Data products**
8. Click **+ New data product**
9. On the **Basic details** step:
   - **Name**: `Customer 360`
   ...
   - **Type**: select `Dataset`
10. Click **Next** → on **Business details**:
...
11. Click **Next** → on **Custom attributes**, skip → click **Create**

**Step 3: Add Assets to the Data Product**

12. After creation, select **Add data assets** → the **Find and select** panel opens with a search box, Collection filters (Databricks Sources, Fabric Sources), and Classification filters
13. Search for and add these assets (you can select multiple before clicking **Add**):
    - **Fabric assets**: `dimension_customer`, `dimension_city`, `dimension_date`
    - **Databricks assets**: `customer` (from samples.tpch), `sales_customers`, `sales_suppliers`

> **Tip**: You can use the **Collection** filter to narrow results to just Fabric Sources or Databricks Sources when multiple results appear.

14. After adding all 6 assets, verify they appear in the data product's **Selected** panel (showing 6 assets)
...
**Expected Result**: `Sales Analytics` governance domain created. `Customer 360` data product created with 6 assets from both Fabric and Databricks.
